...
namespace EasySave_2._0.Models
{
    internal class jobs
    {
        ...
        static public void createJob(CreateBackupViewModel CBVM)
        {


            if (CBVM.JobName == "")
            {
                MessageBox.Show("Name cépabon");
                return;
            }

            if (!System.IO.Directory.Exists(CBVM.PathSource))
            {
                MessageBox.Show("Source cépabon");
                return;
            }

            if (!System.IO.Directory.Exists(CBVM.PathTarget))
            {
                MessageBox.Show("Target cépabon");
                return;
            }

            if (CBVM.PathSource == CBVM.PathTarget)
            {
            ...
            job jb = new job();
            jb.name = CBVM.JobName;
            jb.pathSource = CBVM.PathSource;
            jb.pathTarget = CBVM.PathTarget;
            jb.type = CBVM.Type;

            jobs.listJobs.Add(jb);
